...
def main(argv):
    ...
    ba = BenchmarkAnalyzer('e1_quadruped_perception', hardware_device_type)

    if hardware_device_type == 'cpu':
        # add parameters for analyzing the traces
        ## using message header id
        target_chain = [
            "robotperf_benchmarks:robotperf_pointcloud_input_cb_init",  # 0
            "robotperf_benchmarks:robotperf_pointcloud_input_cb_fini",     # 1
            "robotperf_benchmarks:robotperf_laserscan_output_cb_init",     # 2
            "robotperf_benchmarks:robotperf_laserscan_output_cb_fini"   # 3
        ]
        ...
        ba.add_target(
            {
                "name": "robotperf_benchmarks:robotperf_msg_published_2",
                "name_disambiguous": "robotperf_benchmarks:robotperf_msg_published_2",
                "colors_fg": "red",
                "colors_fg_bokeh": "lightcoral",
                "layer": "userland",
                "label_layer": 4,
        ...
        ba.add_target(
            {
                "name": "robotperf_benchmarks:robotperf_laserscan_input_cb_init",
                "name_disambiguous": "robotperf_benchmarks:robotperf_laserscan_input_cb_init",
                "colors_fg": "red",
                "colors_fg_bokeh": "lightcoral",
                "layer": "userland",
                "label_layer": 4,
        ...
        ba.add_target(
            {
                "name": "robotperf_benchmarks:robotperf_laserscan_input_cb_fini",
                "name_disambiguous": "robotperf_benchmarks:robotperf_laserscan_input_cb_fini",
                "colors_fg": "yellow",
                "colors_fg_bokeh": "darkred",
                "layer": "userland",
                "label_layer": 4,
    ...
    for metric in metrics:
        if metric == 'latency':
            ba.analyze_latency(trace_path, add_power, filter_type='key') #'name' filter if no header keys or 'key' if out of order
...
def generate_launch_description():
    ...
    analyzer = ExecuteProcess(
        cmd=[
            'python3', "src/benchmarks/benchmarks/end_to_end/e1_autonomous_quadruped/launch/analyze_quadruped_perception.launch.py",
