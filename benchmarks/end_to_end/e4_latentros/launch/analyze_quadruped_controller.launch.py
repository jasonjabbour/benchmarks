"""
Analyze: Twist -> JointTrajectory  (CHAMP quadruped_controller — gait + IK)
Session : e4_quadruped_controller

6-tracepoint chain (kernel has msg_received_1 / msg_published_1):
  T_in      = twist_input_cb_init -> msg_received_1
  T_kernel  = msg_received_1 -> msg_published_1
  T_out     = msg_published_1 -> joint_trajectory_input_cb_fini
"""
import sys
import argparse
import json
...
def main(argv):
    ...
    parser.add_argument('--trace_path', type=str,
                        default='/root/.ros/tracing/e4_quadruped_controller')
    ...
    ba = BenchmarkAnalyzer('e4_quadruped_controller', hardware_device_type)
    ...
    for metric in metrics:
        if metric == 'latency':
            ...
            print_latentros_summary(
                ...
                'e4_quadruped_controller', target_chain)
...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
        'trace_path', default_value='/root/.ros/tracing/e4_quadruped_controller'))
    ...
    ld.add_action(ExecuteProcess(
        cmd=[
            'python3',
            'src/benchmarks/benchmarks/end_to_end/e4_latentros/launch/analyze_quadruped_controller.launch.py',
