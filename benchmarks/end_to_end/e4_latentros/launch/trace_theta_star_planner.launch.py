"""
Benchmark: PoseStamped -> Path  (Theta* any-angle planner)
Kernel:    theta_star_benchmark_node  (topic-driven, no lifecycle/action)
Session:   e4_theta_star_planner

Flow:
  goal_pose_publisher.py generates goals at 10Hz
    → /goal_pose → PoseStampedInputComponent
    → /robotperf/benchmark/goal_pose → theta_star_benchmark_node
...
def generate_launch_description():
    ...
    trace = Trace(
        session_name='e4_theta_star_planner',
    ...
    # Kernel: standalone Theta* wrapper (topic-driven, no lifecycle/action)
    kernel = Node(
        package='e4_latentros',
        executable='theta_star_benchmark_node',
        output='screen',
    )
