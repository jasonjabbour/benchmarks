"""
Analyze: PoseStamped -> Path  (Nav2 PlannerServer — Theta Star)
Session : e4_theta_star_planner
"""
import sys
import argparse
import json
...
def main(argv):
    ...
    parser.add_argument('--trace_path', type=str,
                        default='/root/.ros/tracing/e4_theta_star_planner')
    ...
    ba = BenchmarkAnalyzer('e4_theta_star_planner', hardware_device_type)

    # 6-tracepoint chain (now topic-driven with navfn_benchmark_node)
    target_chain = [
        'robotperf_benchmarks:robotperf_pose_stamped_input_cb_init',
        'robotperf_benchmarks:robotperf_pose_stamped_input_cb_fini',
        'robotperf_benchmarks:robotperf_msg_received_1',
        'robotperf_benchmarks:robotperf_msg_published_1',
        'robotperf_benchmarks:robotperf_path_output_cb_init',
        'robotperf_benchmarks:robotperf_path_output_cb_fini',
    ]
    ...
    for metric in metrics:
        if metric == 'latency':
            ...
            print_latentros_summary(
                ...
                'e4_theta_star_planner', target_chain)
...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
        'trace_path', default_value='/root/.ros/tracing/e4_theta_star_planner'))
    ...
    ld.add_action(ExecuteProcess(
        cmd=[
            'python3',
            'src/benchmarks/benchmarks/end_to_end/e4_latentros/launch/analyze_theta_star_planner.launch.py',
