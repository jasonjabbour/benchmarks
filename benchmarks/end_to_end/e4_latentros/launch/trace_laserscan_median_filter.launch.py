"""
Benchmark: LaserScan -> Filtered LaserScan  (laser_filters median filter)
Kernel:    laser_filters scan_to_scan_filter_chain  (with median filter config)
...
def generate_launch_description():
    ...
    median_filter_yaml = os.path.join(config_dir, 'median_filter.yaml')

    trace = Trace(
        session_name='e4_laserscan_median_filter',
    ...
    output_component = ComposableNode(
        ...
        parameters=[{
            'input_topic_name': '/robotperf/benchmark/scan_filtered',
            'output_topic_name': '/scan_filtered',
        }],
        extra_arguments=[{'use_intra_process_comms': False}],
    )
