"""
Analyze: DepthImage -> PointCloud2  (depth_to_pointcloud)
...
6-tracepoint chain (kernel has msg_received_1 / msg_published_1):
  ...
  T_out     = msg_published_1 -> pointcloud_output_cb_fini
"""
import sys
import argparse
import json
...
def main(argv):
    ...
    parser.add_argument('--trace_path', type=str,
                        default='/root/.ros/tracing/e4_depthimage_to_pointcloud')
    ...
    ba = BenchmarkAnalyzer('e4_depthimage_to_pointcloud', hardware_device_type)
    ...
    for metric in metrics:
        if metric == 'latency':
            ...
            print_latentros_summary(
                ...
                'e4_depthimage_to_pointcloud', target_chain)
...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
        'trace_path', default_value='/root/.ros/tracing/e4_depthimage_to_pointcloud'))
    ...
    ld.add_action(ExecuteProcess(
        cmd=[
            'python3',
            'src/benchmarks/benchmarks/end_to_end/e4_latentros/launch/analyze_depth_to_pointcloud.launch.py',
