"""
Analyze: LaserScan -> LaserScan  (laserscan_median_filter)
Session : e4_laserscan_median_filter

6-tracepoint chain (kernel has msg_received_1 / msg_published_1):
  T_in      = laserscan_input_cb_init -> msg_received_1
  T_kernel  = msg_received_1 -> msg_published_1
  T_out     = msg_published_1 -> laserscan_input_cb_fini

Note: input and output both use laserscan_input tracepoints since both
sides use LaserscanInputComponent.
"""
import sys
import argparse
import json
...
def main(argv):
    ...
    parser.add_argument('--trace_path', type=str,
                        default='/root/.ros/tracing/e4_laserscan_median_filter')
    ...
    ba = BenchmarkAnalyzer('e4_laserscan_median_filter', hardware_device_type)

    target_chain = [
        'robotperf_benchmarks:robotperf_laserscan_input_cb_init',
        'robotperf_benchmarks:robotperf_laserscan_input_cb_fini',
    ...
    for metric in metrics:
        if metric == 'latency':
            ...
            print_latentros_summary(
                ...
                'e4_laserscan_median_filter', target_chain)
...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
        'trace_path', default_value='/root/.ros/tracing/e4_laserscan_median_filter'))
    ...
    ld.add_action(ExecuteProcess(
        cmd=[
            'python3',
            'src/benchmarks/benchmarks/end_to_end/e4_latentros/launch/analyze_laserscan_median_filter.launch.py',
