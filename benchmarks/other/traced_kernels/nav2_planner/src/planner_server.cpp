...
PlannerServer::computePlan()
{
  ...
  // LatentROS: extract key from goal pose header
  uint32_t latentros_key = goal->goal.header.stamp.nanosec;
  TRACEPOINT(robotperf_msg_received_1,
      static_cast<const void *>(this),
      static_cast<const void *>(&(*goal)),
      latentros_key);
  ...
  try {
    ...
    // LatentROS: propagate key to result path header
    result->path.header.stamp.nanosec = latentros_key;
...
PlannerServer::publishPlan(const nav_msgs::msg::Path & path)
{
  ...
  // LatentROS: removed all checks — always publish for benchmarking
