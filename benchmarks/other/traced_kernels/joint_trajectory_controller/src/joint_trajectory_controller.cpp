  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  TRACEPOINT(robotperf_msg_received_1,
      ...
      static_cast<const void *>(this),
      latentros_key_.load(std::memory_order_relaxed));
  ...
  const std::shared_ptr<trajectory_msgs::msg::JointTrajectory> msg)
{
  ...
  // LatentROS: store key from trajectory header for propagation in update()
  latentros_key_.store(msg->header.stamp.nanosec, std::memory_order_relaxed);
