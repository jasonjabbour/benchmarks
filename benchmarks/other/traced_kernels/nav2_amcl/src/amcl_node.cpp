...
{
  // LatentROS: extract key stamped by input component
  uint32_t latentros_key = laser_scan->header.stamp.nanosec;
  TRACEPOINT(
      robotperf_msg_received_1,
      static_cast<const void *>(this),
      static_cast<const void *>(laser_scan.get()),
      latentros_key);
  ...
  const std::vector<amcl_hyp_t> & hyps, const int & max_weight_hyp)
{
  ...
  if (!std::isnan(temp)) {
    ...
    // LatentROS: propagate key from input scan to output pose
    p->header.stamp.nanosec = laser_scan->header.stamp.nanosec;
    TRACEPOINT(
