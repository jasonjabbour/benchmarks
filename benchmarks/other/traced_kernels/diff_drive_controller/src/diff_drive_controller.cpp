  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // LatentROS: trace kernel entry with key from header (only when new data)
  uint32_t latentros_key = last_command_msg->header.stamp.nanosec;
  bool latentros_trace_this = latentros_new_data_;
  if (latentros_trace_this) {
    latentros_new_data_ = false;
    TRACEPOINT(robotperf_msg_received_1,
        static_cast<const void *>(this),
        static_cast<const void *>(&(*last_command_msg)),
        latentros_key);
  }
  ...
  // LatentROS: only publish odom when new cmd_vel arrived (event-driven for benchmarking)
  if (latentros_trace_this)
  {
    if (realtime_odometry_publisher_->trylock())
    {
      ...
      odometry_message.header.stamp.nanosec = latentros_key;
  ...
  // LatentROS: trace kernel exit and publish wheel commands for output component
  if (latentros_trace_this) {
    ...
    // Publish wheel commands as JointState so output component can trace them
    if (latentros_cmd_pub_) {
      sensor_msgs::msg::JointState cmd_msg;
      cmd_msg.header.stamp = time;
      cmd_msg.header.stamp.nanosec = latentros_key;
      cmd_msg.name = {"left_wheel_joint", "right_wheel_joint"};
      cmd_msg.velocity = {velocity_left, velocity_right};
      latentros_cmd_pub_->publish(cmd_msg);
    }
  }
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  // LatentROS: publish wheel commands for output component tracing
  latentros_cmd_pub_ = get_node()->create_publisher<sensor_msgs::msg::JointState>(
    "~/joint_commands", rclcpp::QoS(10).reliable());
