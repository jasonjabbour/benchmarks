...
void ControllerServer::computeControl()
{
  ...
  // LatentROS: extract key from path header in the action goal
  auto current_goal = action_server_->get_current_goal();
  if (current_goal && !current_goal->path.poses.empty()) {
    latentros_key_ = current_goal->path.header.stamp.nanosec;
  }
...
void ControllerServer::publishVelocity(const geometry_msgs::msg::TwistStamped & velocity)
{
  ...
  if (vel_publisher_->is_activated() && vel_publisher_->get_subscription_count() > 0) {
    auto cmd_vel_msg = velocity.twist;
    TRACEPOINT(robotperf_msg_published_1,
        static_cast<const void *>(this),
        static_cast<const void *>(&cmd_vel_msg),
        latentros_key_);
